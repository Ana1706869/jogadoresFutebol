...
class JogadoresContentProvider : ContentProvider() {
    private var bdOpenHelper : BdJogadoresOpenHelper? = null
    ...
    override fun onCreate(): Boolean {
        bdOpenHelper = BdJogadoresOpenHelper(context)
        return true
    }
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun query(
        ...
        val tabela = when (endereco) {
            URI_TITULOS, URI_TITULO_ID -> TabelaNTitulos(bd)
            URI_JOGADORES, URI_JOGADOR_ID -> TabelaJogadores(bd)
            else -> null
        }
        ...
        val (selecao, argsSel) = when (endereco) {
            URI_TITULO_ID, URI_JOGADOR_ID -> Pair("${BaseColumns._ID}=?", arrayOf(id))
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun getType(uri: Uri): String? {
        ...
        return when(endereco) {
            URI_TITULOS -> "vnd.android.cursor.dir/$TITULOS"
            URI_TITULO_ID -> "vnd.android.cursor.item/$TITULOS"
 ...
 * and Threads](
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        val endereco = uriMatcher().match(uri)
        val tabela = when (endereco) {
            URI_TITULOS -> TabelaNTitulos(bd)
            URI_JOGADORES -> TabelaJogadores(bd)
            else -> return null
        }
 ...
 * and Threads](
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        val endereco = uriMatcher().match(uri)
        val tabela = when (endereco) {
            URI_TITULO_ID -> TabelaNTitulos(bd)
            URI_JOGADOR_ID -> TabelaJogadores(bd)
            else -> return 0
        }
 ...
 * and Threads](
    ...
    companion object {
        private const val AUTORIDADE = "com.example.jogadoresfutebol"

        const val TITULOS = "NúmeroTítulos"
        const val JOGADORES = "Jogadores"

        private const val URI_TITULOS = 100
        private const val URI_TITULO_ID = 101
        private const val URI_JOGADORES = 200
        private const val URI_JOGADOR_ID = 201

        private val ENDERECO_BASE = Uri.parse("content://$AUTORIDADE")

        val ENDERECO_TITULOS = Uri.withAppendedPath(ENDERECO_BASE, TITULOS)
        val ENDERECO_JOGADORES = Uri.withAppendedPath(ENDERECO_BASE, JOGADORES)

        fun uriMatcher() = UriMatcher(UriMatcher.NO_MATCH).apply {
            addURI(AUTORIDADE, TITULOS, URI_TITULOS)
            addURI(AUTORIDADE, "$TITULOS/#", URI_TITULO_ID)
